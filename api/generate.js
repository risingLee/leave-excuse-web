...
function callHunyuan(prompt) {
  return new Promise((resolve, reject) => {
    ...
    const body = JSON.stringify({
      Model: "hunyuan-lite",
      Messages: [
        { Role: "system", Content: "你是一个职场沟通专家，擅长撰写各种场景下得体、高通过率的请假理由。你熟悉不同国家和企业文化的差异。" },
...
function parseResponse(text) {
  ...
  if (f) result.formal = f[1].replace(/^[（(].*?[)）]\s*/, "").trim();
  if (c) result.casual = c[1].replace(/^[（(].*?[)）]\s*/, "").trim();
  if (e) result.emergency = e[1].replace(/^[（(].*?[)）]\s*/, "").trim();
  if (!result.formal && !result.casual && !result.emergency) {
    const lines = text.split("\n").filter(l => l.trim());
    if (lines.length >= 3) { result.formal = lines[0]; result.casual = lines[1]; result.emergency = lines[2]; }
    else { result.formal = result.casual = result.emergency = text.trim(); }
  }
  return result;
}
...
module.exports = async (req, res) => {
  ...
  const { leaveType, companyStyle, country, extraInfo } = req.body || {};
  if (!leaveType) return res.status(400).json({ error: "请选择请假类型" });

  const prompt = `用户需要请假，请根据以下信息生成3种不同风格的请假理由。
